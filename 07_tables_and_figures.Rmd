...
er <- hw1.5.hn.v$dht$individuals$summary[c(1:10),c(1,5,7,9,10)] %>% 
...
# abundance
Nhat_t <- hw1.5.hn.v$dht$individuals$N[c(1:10),c(1,2,4,5,6)] %>% 
...
# density
d <- hw1.5.hn.v$dht$individuals$D[c(1:10),c(1,2,5,6)]  %>% 
  transmute(Season=Label,
...
model.table.hw <- summarize_ds_models(
  ...
  mutate(
    # Formula=gsub("Clumped_","",Formula),
         ...
         Formula=gsub("90c","",Formula)) %>% dplyr::select(-Model)
...
# encounter rate and group size
er <- hp.0.65.hn.bfc.obs$dht$individuals$summary[c(1:10),c(1,5,7,9,10)] %>% 
  transmute(Season=Region,
            n     =round(n,         digits=0), 
            ER    =round(ER,        digits=2),
            cv.ER =round(cv.ER,     digits=2),
            GS=    round(mean.size, digits=2))
# abundance
Nhat_t <- hp.0.65.hn.bfc.obs$dht$individuals$N[c(1:10),c(1,2,4,5,6)] %>% 
  transmute(Season=Label,
            N=    round(Estimate,digits=0), 
            cv.N= round(cv,      digits=2),
            L95.N=round(lcl,     digits=0),
...
# density
d <- hp.0.65.hn.bfc.obs$dht$individuals$D[c(1:10),c(1,2,5,6)] %>% 
  transmute(Season=Label,
            D =    round(Estimate*25,digits=2),
            # cv.D = round(cv,         digits=2),
            L95.D= round(lcl*25,     digits=2),
...
dp0.8.hn.swc.obs,
output="plain")
# model.table.oct.allsgt0.8 <- summarize_ds_models(dp0.8.hn.bfc ,
#                      dp0.8.hn.szc ,
#                      # dp0.8.hn.szc2,
#                      dp0.8.hn.g90 ,
#                      dp0.8.hn.g90y,
...
#                      # dp0.8.hn.swc..obs,
#                      # dp0.8.hn.swc.g45c    ,
#                      # dp0.8.hn.swc.g45c.bfc,
#                      # dp0.8.hn.swc.g45c.szc,
#                      # dp0.8.hn.swc..g45c,
#                      output="plain")
...
# encounter rate and group size
er <- dp0.8.hn.swc.g90c$dht$individuals$summary[c(1:10),c(1,5,7,9,10)]  %>% 
  transmute(Season=Region,
            n     =round(n,     digits=0), 
            ER    =round(ER,    digits=2),
            cv.ER =round(cv.ER, digits=2),
            GS=round(mean.size, digits=2))
# abundance
Nhat_t <- dp0.8.hn.swc.g90c$dht$individuals$N[c(1:10),c(1,2,4,5,6)] %>% 
  transmute(Season=Label,
            N=    round(Estimate,digits=0), 
            cv.N= round(cv,      digits=2),
            L95.N=round(lcl,     digits=0),
            U95.N=round(ucl,     digits=0))
# density
d <- dp0.8.hn.swc.g90c$dht$individuals$D[c(1:10),c(1,2,5,6)] %>% 
  transmute(Season=Label,
...
study_area <- ggplot() +
  ...
  #         fill=NA,alpha=0.3)+
  geom_sf_pattern(data = survey_can, fill = NA, colour='grey50', size=2,aes(pattern_fill = "Study\nArea"),
  ...
  theme(legend.position = "bottom",
        ...
        legend.spacing = unit(c(-0.4,-0.3,-0.4), "lines"),
...
sp <- "humpback"
 plot_survey(
  ...
  season=F,
  by_seasonYear = T)
  # ggsave(paste0("knitr-figs-pdf/",sp,"_seasonYear_stg_map.png"))
  # knitr::include_graphics(paste0("knitr-figs-pdf/sightings_seasonYear_hw_2023apr_black.png"))
...
sp <- "harbour porpoise"
 plot_survey(
  ...
  season=F,
  by_seasonYear = T)
  # ggsave(paste0("knitr-figs-pdf/",sp,"_seasonYear_stg_map.png"))knitr::include_graphics(paste0("knitr-figs-pdf/sightings_seasonYear_hw_2023apr_black.png"))
# knitr::include_graphics(paste0("knitr-figs-pdf/sightings_seasonYear_hp_2023apr_black.png"))
...
sp <- "Dall's porpoise"
 plot_survey(
  ...
  by_seasonYear = T)
  # ggsave(paste0("knitr-figs-pdf/",sp,"_seasonYear_stg_map.png"))
  # knitr::include_graphics(paste0("knitr-figs-pdf/sightings_seasonYear_hw_2023apr_black.png"))plot_sp_monthly()
```
...
plot_ab_hp <- ab_hp[1:9,] %>% dplyr::select(SeasonYear, "Estimated Abundance"=N, L95.N, U95.N) %>% mutate(Species = "Harbour porpoise")
plot_ab_dp <- ab_dp[1:9,] %>% dplyr::select(SeasonYear, "Estimated Abundance"=N, L95.N, U95.N) %>% mutate(Species = "Dall's porpoise")
plot_ab_hw <- ab_hw[1:9,] %>% dplyr::select(SeasonYear, "Estimated Abundance"=N, L95.N, U95.N) %>% mutate(Species = "Humpback whale")
 lev <- levels(all_ap_sf$Species) %>% first_up()
...
shape <- c('Humpback whale' = 23,
           'Harbour porpoise' = 24,
           'Dall\'s porpoise' = 22)
ggplot(plot_ab, aes(SeasonYear, `Estimated Abundance`), colour="black") +
  ...
  theme(legend.text = element_text(size=fig_legend_size),
        ...
        axis.text.x = element_text(angle=90))+
  ggtitle("Winter = Feb-Apr, Spring = May-Jul, Summer = Aug-Oct, Fall=Nov-Jan")
ggsave("C:/users/keppele/documents/cemore/analysis/cemore_analysis/output_cemore/diff seasons/late seasonYear abund.png")
