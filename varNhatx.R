# df_unc <- lapply(model=ddf[[1]], Distance:::varNhat, data=res)

varNhatx <- function(data, model){
  # model=ddf[[1]]
  # data=res
  # format the data
  ...
  grp_dat <- data %>%
    dplyr::select("Covered_area", "Area", "Sample.Label", !!strat_vars) %>%
    distinct() %>%
  ...
  # data<- data[!is.na(data$object), ] # EK edit - remove so the right number of rows remain in summary dataframe

  # calculate variance
  dm <- DeltaMethod(model$par, dhtd, vcov, sqrt(.Machine$double.eps),
                    model=model, data=data)
  # dm2 <- DeltaMethod(model$par, dhtd, vcov, sqrt(.Machine$double.eps),
                    # model=model, data=data2)
