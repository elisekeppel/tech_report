...
# TO DO : fix in main dataset
all_effort_lines[which(all_effort_lines$TransectID=="cemore_2022apr7" ),]$month <- 4
...
all_ap_sf <- all_ap_sf %>%
  ...
  dplyr::mutate(Beaufort = as.numeric(as.character(Beaufort)),
                ...
                Observer = case_when(
                  ...
                  Observer == "EKeppel"~"b",
                  Observer == "LSpaven"~ "c",
                  Observer == "ChristieMcMillan" ~"a",
                  Observer == "EliseKeppel"~"b",
                  Observer == "LisaSpaven"~ "c"),
  ...
  mutate(
    ...
    ## shift seasons to start one month earlier
    ...
    # seasonYear = paste0(tolower(season),
    #   case_when(
    #     month %in% c(1:11) ~ year,
    #     month == 12        ~ year+1)),
    
    ## shift seasons to start one month later
    # season = factor(dplyr::case_when(
...
## previous function created to be run within purrr::map to apply to each survey individually
...
# order_transects <- function(){
# x <- all_effort_lines %>%
  # filter(SurveyID==sid) %>%
  # dplyr::select(TransectID) # SurveyID

van <- data.frame(x = -123.3, y = 49.4) %>%
  sf::st_as_sf(coords = c("x","y"), crs=4326)
d <- as.vector(round(st_distance(van, all_effort_lines$geometry)/1000,2))

new_tids <- all_effort_lines %>% dplyr::mutate(dist = d) %>%
  st_drop_geometry() %>%
  group_by(TransectID) %>% #SurveyID
  summarise(dist=min(dist)) %>%
  ungroup() %>%
  arrange(dist) %>%
  mutate(order = 1:length(unique(TransectID))) %>% dplyr::select(-dist)
         # new_TID = paste0(SurveyID, order),
         # new_TID = factor(new_TID, levels = new_TID))

# previously ran this as a function in purrr::map() to apply to each survey individually
# rm(new_tids)
# new_tids <- map_df(sid_list, order_transects) #%>% dplyr::select(-dist)

all_effort_lines <- all_effort_lines %>% full_join(new_tids) %>% #mutate(TransectID=order, new_TID = NULL) %>%
  arrange(order)
all_ap_sf <- all_ap_sf %>% left_join(new_tids)# %>% mutate(TransectID=new_TID, new_TID = NULL)
all_ap_sf %<>%   mutate(Sample.Label=TransectID) %>% arrange(TransectID)#update Sample.Label w new tids
#------------------------------------------------
