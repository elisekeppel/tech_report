dht2y <- function(ddf, observations=NULL, transects=NULL, geo_strat=NULL,
                 flatfile=NULL,
  ...
  res_sample$distance <- res_sample$object <- res_sample$size <- res_sample$Nhat <- NULL # else $size gets propagated through to group_mean and group_var, etc., except it doesn't cuz res_sample is no longer used ...
  ...
  df_unc <- lapply(ddf, varNhatx, data=res)
    ...
    if(length(stra_row[[this_stratum]]) > 1){
      ...
      if(stratification=="geographical"){
        ...
        dat_row <- dat_row %>%
          ...
          mutate(ER_var       = sum((.data$Effort/sum(.data$Effort))^2*  # sum of effort-weighted ER_var per stratum
                                      .data$ER_var,
                                    na.rm=TRUE)) %>%
          mutate(ER_var_Nhat  = sum(.data$weight^2*.data$ER_var_Nhat,
                                    na.rm=TRUE)) %>% # sum of effort-weighted ER_var_Nhat per stratum
      ...
      dat_row <- dat_row %>%
        mutate(n = sum(.data$n)) %>%
        mutate(ER = .data$n/.data$Effort) %>% # overall ER
        mutate(ER_CV = if_else(.data$ER==0,  # summed weighted ER_var/overall ER
                               0,
                               sqrt(.data$ER_var)/.data$ER)) %>%
        mutate(group_mean     = mean(.data$group_mean), # mean of group means by stratum, not weighted
               group_var      = sum(.data$group_var),   # sum of group var by stratum, not weighted
               group_var_Nhat = sum(.data$group_var_Nhat)) %>% # sum of group_var_Nhat by stratum, not weighted
        mutate(group_CV = if_else(.data$group_var==0, 0,
                                  sqrt(.data$group_var)/.data$group_mean)) # summed group var / mean group mean
      ...
      # detection function CV ### EK note: potential big problem here - 'Total' Abundance hasn't been calc'd yet for "replicate"...
      dat_row <- dat_row %>%
      ...
      if(stratification=="replicate"){
        ...
        rvar <- sum(dat_row$weight*(dat_row$Abundance - # EK note: should this abund be weighted before subtracting summed weighted abund?)
